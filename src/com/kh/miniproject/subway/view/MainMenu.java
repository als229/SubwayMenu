...
public class MainMenu {
	...
	private SandwichController sub = new SandwichController();  // SandwichController 의 메서드들을 사용하기 위해 SandwichController 타입의 객체 생성
	...
	public void mainMenu() {
		...
		while (true) { // 빵 주문을 잘못 입력했을 경우 다시 돌아와야 하기 때문에 무한반복문으로 만들어 줬습니다.
			...
			int myCoin = Integer.parseInt(sc.nextLine()); // nextInt를 사용하면 46행에 nextLine이 작동하지 않기 때문에 nextLoine으로 사용한 후 parseInt로 integer로 바꿔주었습니다.
			int menuPrice = 0; //메뉴 가격을 담을 변수 menuPrice를 선언과 0으로 초기화를 해줬습니다.
			System.out.println();
			if (myCoin < 4000) {
				System.out.println("4000원 이하는 나가주시죠 ㅋㅋ~~zzz");
				return; // 현재 고객이 가진 금액을 myCoin으로 입력받아서 가장 적은 메뉴 가격은 4000원 미만일때는 return으로 프로그램이 종료되게 하였습니다.
			} else {
			...
			if (setbool.equals("네")) { // String 타입 setbool에 입력받은 값이 "네" 이면 equals를 사용해 스트링타입끼리 비교하여 둘이 같으면 set를 true로 바꾸어
			...
			String[] realvegetable = new String[vegetableView.length - count];// 이제 사용자가 빼고싶은 야채값이 없는 야채를 담을 배열을 만들어 줍니다. 길이는 vegetableView의 길이에서 
			for (int i = 0; i < vegetableView.length; i++) {   					// 빼고싶은 야채수인 count를 빼주워서 생성했습니다. 그 후 vegetableView의 길이만큼 for문을 돌려줍니다. 
				for (int j = 0; j < realvegetable.length; j++) {   // realvegetable의 크기만큼 돌려줄 for문을 만들어준 후
					if (vegetableView[i] != "") {  //if 문을 넣어서 vegetableView 에 공백이 아닐 경우 realvegetable에 vegetableView의 내용을 옮겨 담아 줍니다.
			...
			if (set) { //set가 true일때만 실행되게 하였습니다.
				...
				sub.insertMenu(name, menuPrice, bread, cheese, vegetable, sauce, cookie, drink); // SandwichController에 있는 insertMenu 메서드를 쓰기위해 만든 SandwichController
																								// 타입의 변수 sub를 이용해 메서드를 호출과 동시에 초기화를 해주었습니다.
				System.out.println("\n주문하신 " + name + "세트 나왔습니다.");
				System.out.println("\n" + sub.printMenu() + "\n쿠키는 " + cookie + ", 음료는 " + drink + "입니다."); //insertMenu에서 초기화를 해준 후 printMnu를 통해 출력문을 출력해 주었습니다.
